...
class BinlookupAPIClientError(Exception):
...
class ValidationError(BinlookupAPIClientError):
...
class BinlookupAPIClient:
    ...
    VALIDATION_RULES = {"bin": {"type": "string", "required": True, "minLength": 6, "maxLength": 8}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the BinlookupAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises BinlookupAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[BinlookupAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns BINLookupData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises BinlookupAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise BinlookupAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise BinlookupAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise BinlookupAPIClientError(f"Request failed: {str(e)}")
