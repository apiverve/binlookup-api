...
class BinlookupValidationException implements Exception {
  final List<String> errors;

  BinlookupValidationException(this.errors);

  @override
  String toString() => 'BinlookupValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/binlookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [bin] (required) - The first six digits of the credit card number (BIN) [minLength: 6, maxLength: 8]
class BinlookupClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'bin': ValidationRule(type: 'string', required: true, minLength: 6, maxLength: 8),
  };

  BinlookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/binlookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BinlookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BinlookupValidationException(errors);
    }
  }

  /// Execute a request to the BIN Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<BinlookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BinlookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BinlookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BinlookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BinlookupException('Resource not found');
      } else {
        throw BinlookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BinlookupException) rethrow;
      throw BinlookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the BIN Lookup API client.
class BinlookupException implements Exception {
  final String message;

  BinlookupException(this.message);

  @override
  String toString() => 'BinlookupException: $message';
}
